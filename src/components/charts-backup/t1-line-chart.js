...
export const TOneLineChart = ({ height = 350 }) => {
    let svgRef = useRef(),
        toolRef = useRef(),
        width = 800,
    ...
    useEffect(() => {
        ...
        let xScale = d3.scaleLinear().domain(d3.extent(data, function (d) { return d.year })).range([0, width]),
            yScale = d3.scaleLinear().domain([0, d3.max(data, function (d) { return d.n })]).range([height, 0]);
